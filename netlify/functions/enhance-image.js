...
- Keep the main subject, apply magazine-grade photography techniques`
    ...
    // Call Gemini Vision API for image enhancement
    ...
    // Note: Gemini 2.0 Flash can generate images, returns as base64
    const generatedImage = data.candidates?.[0]?.content?.parts?.find(p => p.inline_data)?.inline_data?.data;

    if (!generatedImage) {
      // If no image was generated, provide text enhancement instructions
      ...
      return {
        ...
        body: JSON.stringify({
          success: true,
          enhancedImage: null,
          enhancementInstructions: textResponse,
          message: 'Image generation not supported by this model. Enhancement instructions provided.'
        })
      };
    }
